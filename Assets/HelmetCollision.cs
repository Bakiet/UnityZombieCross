...
public class HelmetCollision : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D obj)
	{
		if (obj.tag == "Ground") {
			...
			if (obj.tag != "Checkpoint" ^ obj.tag != "ZoomOutTrigger" ^ obj.tag != "ZoomInTrigger" ^ obj.tag == "Player" ^ obj.tag == "Coin" ^ obj.tag == "nitro") { //if entered in any other trigger than "Finish" & "Checkpoint", that means player crashed
