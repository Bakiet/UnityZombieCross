...
public class Ferr2D_PathEditor : Editor { 
	Texture2D texMinus           = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-minus.png"         );
	Texture2D texMinusSelected   = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-minus-selected.png");
	Texture2D texDot             = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot.png"               );
	Texture2D texDotSnap         = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-snap.png"          );
	Texture2D texDotPlus         = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-plus.png"          );
	Texture2D texDotSelected     = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-selected.png"      );
	Texture2D texDotSelectedSnap = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-selected-snap.png" );
	
	Texture2D texLeft   = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-left.png"  );
	Texture2D texRight  = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-right.png" );
	Texture2D texTop    = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-top.png"   );
	Texture2D texBottom = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-down.png"  );
	Texture2D texAuto   = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-auto.png"  );
	Texture2D texReset  = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-reset.png" );
	Texture2D texScale  = Ferr.EditorTools.GetGizmo("2D/Gizmos/dot-scale.png" );
	...
	private void    DoHandles            (Ferr2D_Path path, GUIStyle iconStyle)
	{
		...
		for (int i = 0; i < path.pathVerts.Count; i++)
		{
			...
			if (Event.current.alt)
			{
				...
				if (Handles.Button(posOff, SceneView.lastActiveSceneView.camera.transform.rotation, handleScale, handleScale, cap))
				{
					if (!isSelected) {
						selectedPoints.Clear();
						selectedPoints.Add(i);
					...
					if (terrain && i+1 < path.pathVerts.Count) {
						...
						if (IsVisible(posDirOff)) {
							...
							if (Handles.Button(posDirOff, SceneView.lastActiveSceneView.camera.transform.rotation, scale, scale, cap)) {
				...
				if (terrain) {
					...
					if (Ferr2DT_SceneOverlay.editMode == Ferr2DT_EditMode.Override && i+1 < path.pathVerts.Count) {
						...
						if (IsVisible(posDirOff)) {
							...
							Vector3 scaleMove = Handles.FreeMoveHandle(posDirOff, SceneView.lastActiveSceneView.camera.transform.rotation, scale, Vector3.zero, cap);
				...
				if (i + 1 < path.pathVerts.Count || path.closed == true) {
					...
					if (Handles.Button(mid + offset, SceneView.lastActiveSceneView.camera.transform.rotation, handleScale, handleScale, CapDotPlus)) {
	...
	private void    DoShiftAdd           (Ferr2D_Path path, GUIStyle iconStyle)
	{
		...
		if (IsVisible(handlePos)) {
			if (Handles.Button(handlePos, SceneView.lastActiveSceneView.camera.transform.rotation, HandleScale(handlePos), HandleScale(handlePos), CapDotPlus))
			{
