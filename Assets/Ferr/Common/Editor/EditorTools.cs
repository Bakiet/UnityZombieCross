...
namespace Ferr {
	public class EditorTools {
		...
		public static void ImageCapBase(int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, Texture2D aTex)
		{
			...
			Material imageCapMat = new Material(Shader.Find("Hidden/Ferr Gizmo Shader 2D"));
			...
			imageCapMat.mainTexture = aTex;
			imageCapMat.SetPass(0);
			...
			GameObject.DestroyImmediate(imageCapMat);
		}
		public static void CubeCapDirBase(int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, Vector3 aScale) {
			if (Event.current.type != EventType.Repaint) {
				return;
			}
			Material imageCapMat = new Material(Shader.Find("Hidden/Ferr Gizmo Shader 3D"));
			
			aPosition  = Handles.matrix.MultiplyPoint(aPosition);
			imageCapMat.SetPass(0);
			
			for (int i = 0; i < imageCapMat.passCount; ++i) {
				imageCapMat.SetPass(i);
