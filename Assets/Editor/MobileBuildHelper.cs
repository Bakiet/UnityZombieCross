...
public class MobileBuildHelper
{
    ...
    {
        get { return EditorPrefs.GetString("AndroidSdkRoot") + "/platform-tools/"; }
    }
    ...
    [MenuItem("Build Helper/Push To Android (no -d)")]
    public static void PushToAndroidWithNoForce()
    {
        PushToDevice(false);
    }

    private static void PushToDevice(bool useForceFlag)
    {
        ...
        var installOnDeviceProcess = new ProcessStartInfo
        {
            FileName = AndroidSdkPath + "adb",
            Arguments = string.Format("install -r {0} \"{1}\"", useForceFlag ? "-d" : string.Empty, apkLocation)
        };

        if (ExecuteProcessAndHandleErrors(installOnDeviceProcess) != null)
        {
            var runOnDeviceProcess = new ProcessStartInfo
            {
                FileName = AndroidSdkPath + "adb",
                Arguments =
                    string.Format("shell am start -n \"{0}/{1}\"", PlayerSettings.bundleIdentifier,
                        "com.unity3d.player.UnityPlayerNativeActivity")
            };

            ExecuteProcessAndHandleErrors(runOnDeviceProcess);
        }
        else
        {
    ...
    private static Process ExecuteProcessAndHandleErrors(ProcessStartInfo processInfo)
    {
        ...
        if (process != null)
        {
            ...
            error = !string.IsNullOrEmpty(error) ? error : output.Contains("Error") ? output : null;

            if (!string.IsNullOrEmpty(error))
            {
                if (Regex.IsMatch(error, @"\d+\sKB\/s\s\(\d+\sbytes\sin\s\d+.?\d*s?\)"))
                {
                    if (output.Contains("[INSTALL_CANCELED_BY_USER]"))
                    {
                        error = "Installation was cancelled by the user or the screen is off";
                    }
    ...
    public static void ExtractKeysFromKeystore()
    {
        ...
        var extractKeysProcessInfo = new ProcessStartInfo
        {
            FileName = "cmd",
        ...
        var extractKeysProcess = ExecuteProcessAndHandleErrors(extractKeysProcessInfo);

        if (extractKeysProcess != null)
        {
            var output = extractKeysProcess.StandardOutput.ReadToEnd();
            ...
            ExecuteProcessAndHandleErrors(new ProcessStartInfo
            {
                FileName = "cmd",
