...
namespace SIS
{
    ...
    public class RequirementEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            //draw fields for storage entry id and its target to reach
            //when an entry has been entered in its field, color them yellow (active) 
            if (!string.IsNullOrEmpty(obj.req.entry))
                GUI.backgroundColor = Color.yellow;
            obj.req.entry = EditorGUILayout.TextField(new GUIContent("Storage Entry:", "Entry in storage which stores the required " +
            ...
            obj.req.labelText = EditorGUILayout.TextField(new GUIContent("Locked Label:", "Text that describes what the player has to do to fulfill " +
                                                                         "the requirement. Displayed in the prefab's 'Locked Label' component, if set"), obj.req.labelText);
